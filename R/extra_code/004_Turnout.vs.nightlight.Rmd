...
---
```{r}
here::i_am("R/004_Turnout.vs.nightlight.Rmd")
require(lmtest)
require(plm)
require(stargazer)
...
to.model <- turnout %>% left_join(mean_nightlight,by=c("index","year"))  %>% rename(region=label) %>% select(-index)
...
```{r}
m <- model_1_1 <- plm(turnout ~ nightlight_mean,data=to.model,model="within",effect = "twoways")
...
pcdtest(turnout ~nightlight_mean,data=to.model,model = "within",effect="twoways",w=nb.matrix)
...
plm::pwartest(turnout ~nightlight_mean,data=to.model)
...
```{r}
mpool <- plm(turnout ~nightlight_mean,data=to.model,model="pooling")
...
```{r}
m <- model_1_2 <-  plm(turnout ~ nightlight_mean,data=to.model,model="within",effect = "twoways")
...
```{r}
m <-  model_1_3 <-  plm(turnout ~ nightlight_mean,data=to.model,model="within",effect = "twoways")
...
out.file <- here::here("tables/turnout.vs.nightlight_models.html")
