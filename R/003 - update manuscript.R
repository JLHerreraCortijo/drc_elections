...
model_t3a_1_1 <- plm(votes_share ~ nightlight_mean +n.deaths,data=to.model,model="within",effect = "twoways")
...
models.to.print <- list(model_t3a_1_1,model_t3a_1_2,model_t3a_1_3,model_t3a_1_4,model_t3a_1_5,model_t3a_1_6,model_t3a_1_7,model_t3a_1_8,
                        model_t3a_1_9,model_t3a_1_10,model_t3a_1_11,model_t3a_1_12,model_t3a_1_13,model_t3a_1_14,model_t3a_1_15,model_t3a_1_16)

model_names <- c("Total Deaths","Log\nTotal Deaths","Total Conflicts","Log\nTotal Conflicts","Total Deaths\nno NL","Log Total Deaths\nno NL","Total Conflicts\nno NL","Log Total Conflicts\nno NL",
...
Table_3a <- suppressWarnings(stargazer::stargazer(models.to.print,type="html",
...
Table_3a <- read_html(Table_3a) %>% html_table() %>% as.data.frame() %>% slice(-c(1:6))
table_names <- c(" ",model_names)

Table_3a <- set_names(Table_3a,table_names) 

empty_lines <- Table_3a %>% transmute(across(everything(),~nchar(.)==0)) %>% rowwise() %>% transmute(all(c_across(everything()))) %>% pull(1) %>% which()

Table_3a %<>% slice(-empty_lines)

Table_3a %<>% flextable() %>% merge_at(i=nrow(Table_3a),j=2:ncol(Table_3a)) %>%
  style(pr_t=fp_text(font.size = 11),part = "all",pr_p = fp_par(line_spacing = 1,padding = 0,text.align="center"))  %>% 
  style(pr_t=fp_text(font.size = 11),part = "header",pr_p = fp_par(line_spacing = 1,padding = 0,text.align="center"))  %>% 
  hline(i = 10,border = officer::fp_border()) %>%
  vline(j=c(1,9),border=officer::fp_border())%>% autofit() %>% flextable::fit_to_width(10.5)
